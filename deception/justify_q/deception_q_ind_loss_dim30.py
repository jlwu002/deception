...
def calculate_loss(dim_single,div_num,num_sim):
    ...
    for k in range(1000):
        if k%100==0:
            ...
            text_file = open("ind_"+str(dim_single)+"_"+str(k)+".txt", "w")
...
np.savetxt("ind_loss_q_dim"+str(dim_single)+".csv", loss_list_q, delimiter =", ", fmt ='% s')
np.savetxt("ind_loss_z_dim"+str(dim_single)+".csv", loss_list_z, delimiter =", ", fmt ='% s')

np.savetxt("ind_timecount_dim"+str(dim_single)+".csv", [time_count], delimiter =", ", fmt ='% s')