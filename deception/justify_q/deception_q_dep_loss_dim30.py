...
def loss_fun(fx, fy, fz,exploits,div_num,dim_single,dim,num_x):
    #if y = 0, mask; if y = 1, nonmask
    c = 0.05*(dim - torch.sum(fy,1))
    ...
    v = torch.sum(x_2_expand_sub+1,1)/2    
...
def loss_fun_z(fx, fy, fz,exploits,div_num,dim_single,dim,num_x):
    x_2 = fx.repeat_interleave(exploits.size(0), dim = 0)
    x_2_expand = x_2.reshape(num_x*div_num*exploits.size(0),dim_single)
    x_2_expand_sub = x_2_expand[:,:int(dim_single/2)]
    v = torch.sum(x_2_expand_sub+1,1)/2    
...
def calculate_loss(dim_single,div_num,num_sim):
    ...
    f = open('exploit_'+str(dim_single)+'_q_dep.pckl', 'rb')
    ...
    for k in range(1000):
        if k%100==0:
            ...
            text_file = open("dep_"+str(dim_single)+"_"+str(k)+".txt", "w")
        ...
        optimizer_z = torch.optim.Adam(nnz.parameters(), lr=0.05, betas=(0.5, 0.999))
        ...
        for ii in range(div_num):    
            ...
            #generate system
            #-1 if not installed, otherwise version 1-5
            x_rand_sys_idx = np.random.randint(0, 3, num_x)
            x_rand_sys = -np.ones((num_x, 3))
            
            for idx in range(num_x):
                x_rand_sys[idx][x_rand_sys_idx[idx]]=1
                
            #generate application
            app_dim = int(dim_single/2) - 3
            x_rand_app = np.random.randint(0, 2, (num_x, app_dim))
        ...
        for i in range(201):   
            ...
            for epoch in range(3):
                ...
                z = nnz(yy)
                z_loss = -loss_fun_z(x,y,z,exploits,div_num,dim_single,dim,num_x)
            ...
            yy = y*x
            z = nnz(yy)
            q_loss = loss_fun(x,y,z,exploits,div_num,dim_single,dim,num_x) 
            #if (i%100==0):
             #   print(q_loss)
                # print(torch.min(y))
                # print(torch.max(y))
...
np.savetxt("dep_loss_q_dim"+str(dim_single)+".csv", loss_list_q, delimiter =", ", fmt ='% s')
np.savetxt("dep_loss_z_dim"+str(dim_single)+".csv", loss_list_z, delimiter =", ", fmt ='% s')

np.savetxt("dep_timecount_dim"+str(dim_single)+".csv", [time_count], delimiter =", ", fmt ='% s')